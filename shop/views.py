...
def search(request):
    ...
    products = Product.objects.filter(category__name__icontains=query) | Product.objects.filter(
        name__icontains=query) | Product.objects.filter(description__icontains=query) | Product.objects.filter(
                    ...
                    if session_cart:
                        for item_id, item in session_cart.items():
                            ...
                            if CartItem.objects.filter(product=product, cart=cart):
                                ...
                                new_item = CartItem(product=product, price=float(item['price']),
                                                    quantity=int(item['quantity']), cart=cart)
                                new_item.save()
